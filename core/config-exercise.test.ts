...
test("exercise year, states, capacity thresholds and transverse domains: parsed, defaulted, refused when malformed", () => {
  ...
  const cases: [string, (raw: any) => void][] = [
    ...
    ["doneStates not a list", (raw) => (raw.exercise.doneStates = "Terminé")],
    ["doneStates empty", (raw) => (raw.exercise.doneStates = [])],
    ["done state outside the retained states", (raw) => { raw.exercise.states = ["Budget validé"]; raw.exercise.doneStates = ["Terminé"]; }],
