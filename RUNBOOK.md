...
|---|---|
| `Cout.csv` (export « Coût », COUT PREV) | **Le périmètre depuis le 2026-09-11 (ADR 030)** : l'export brut Sciforma des coûts prévisionnels (une ligne par projet × centre de coût × année). Projets uniques de l'exercice, état dans la liste `exercise.states` de la config, type parmi les types de la config, lignes « arbitrage » écartées, projets sans aucun chiffre ME écartés ; domaine lu dans « Projet.Portefeuille » ; les montants ne servent qu'à ce test, jamais lus dans les cartes ; Responsable jamais lu. Quand il est là, `Projets.csv` ne sert qu'au recoupement (ligne « périmètre · recoupement » du rapport : codes présents d'un seul côté). |
