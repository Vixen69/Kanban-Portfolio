// Reader for the raw `projet` export: the chef de projet (Responsables
// 1→2→3 minus the RDOM names) and the domain FALLBACK via « Responsable
// portefeuilles » matched against the RDOM table by surname
// (docs/IMPORT-MAPPING.md, Q4+Q5). The consolidated sheet stays the
// perimeter master; this file only enriches.
...
interface ProjetsContext {
  ...
  report: ImportReport;
  fileName: string;
  /** Normalized RDOM surname -> domain ids (ambiguous surnames -> >1). */
  surnames: Map<string, Set<string>>;
  entries: ProjetEntry[];
...
export function parseProjets(
  ...
  for (const entry of rdom?.entries ?? []) {
    const set = surnames.get(entry.normalizedName) ?? new Set<string>();
    set.add(entry.domainId);
    surnames.set(entry.normalizedName, set);
  }
  if (rdom === null) {
    warn(report, "table RDOM absente — domaine par responsable de portefeuille non résolu", fileName);
  }
  const ctx: ProjetsContext = {
    match, report, fileName, surnames,
...
// The domains of every RDOM surname present in the cell, matched on whole
...
function rdomDomains(ctx: ProjetsContext, value: string): Set<string> {
  const words = new Set(normalizeLabel(value).split(/[^a-z0-9']+/));
  const domains = new Set<string>();
  for (const [surname, ids] of ctx.surnames) {
    if (words.has(surname)) for (const id of ids) domains.add(id);
  }
  return domains;
}
