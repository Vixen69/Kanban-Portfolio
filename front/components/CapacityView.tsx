// Capacity view (☷, ADR 024/025): the arbitration read-out between domain
// owners — where the exercise year's planned demand lands against the
// people's declared capacity, with the transverse domains (A&D, INFRA)
// singled out. Every figure comes from core/capacity-view.ts; this file
// fetches the snapshot when the view opens, memoises the computation and
// lays out the panels of ./capacityPanels.tsx and ./capacityTables.tsx.

import { useEffect, useMemo, useState } from "react";
import type { BoardConfig, CapacitySnapshot, CardState } from "../../core/types.ts";
import { computeCapacityReadout, type CapacityReadout } from "../../core/capacity-view.ts";
...
// Global load: past the capacity is an alert, close to it a warning.
...
function Kpis({ readout }: { readout: CapacityReadout }) {
  ...
  return (
    <div className="m2-kpis">
      ...
      <Kpi num={fmtUnit(kpis.demandJh)} unit="j.h" label="Demande prévisionnelle" tone="accent" />
      <Kpi num={pct(kpis.ratio)} label="Charge globale" tone={ratioTone(kpis.ratio)} />
      <Kpi num={kpis.overloaded} label="Personnes au-delà de 100 %" tone={kpis.overloaded > 0 ? "alert" : "ok"} />
      <Kpi num={kpis.cardsWithoutAssignment} label="Cartes sans affectation" tone={kpis.cardsWithoutAssignment > 0 ? "warn" : "ok"} />
      <Kpi num={fmtUnit(kpis.doneJh)} unit="j.h" label="Réalisé (indicatif)" />
    </div>
  );
}
  ...
  return (
    <div className="m2-panel wide">
      ...
      <div className="m2-note">
        ...
        classeur, lancer l’import avec l’option « --charger ». Les personnes et leurs affectations
        de l’exercice apparaissent alors ici, contre leur capacité déclarée.
      </div>
    </div>
  );
}

function Body({ fetch, cards, config }: { fetch: Fetch; cards: CardState[]; config: BoardConfig }) {
  const snapshot = fetch.status === "ready" ? fetch.snapshot : null;
  const readout = useMemo(
    () => (snapshot === null ? null : computeCapacityReadout(snapshot, cards, config)),
    [snapshot, cards, config],
  );
...
function subtitle(fetch: Fetch): string {
  if (fetch.status !== "ready" || fetch.snapshot === null) return "Lecture annuelle : prévisionnel de l’exercice contre capacité déclarée";
  const { exerciseYear, persons, assignments } = fetch.snapshot;
  return `Exercice ${exerciseYear} · ${persons.length} personnes · ${assignments.length} affectations · prévisionnel annuel contre capacité déclarée`;
}

/**
 * Full-screen capacity view (☷).
 * Inputs: CapacityViewProps — the active cards, the runtime config, the
 * close callback. Output: the overlay DOM.
...
export function CapacityView(props: CapacityViewProps) {
  ...
  return (
    <div className="metrics-view m2">
      ...
      <Body fetch={fetch} cards={props.cards} config={props.config} />
    </div>
  );
}
